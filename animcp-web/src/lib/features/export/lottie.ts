...
export function validateExportSettings(settings: ExportSettings) {
	if (
		...
		throw new Error('Export duration must be between 1 and 10 seconds.');
	if (!exportFrameRates.includes(settings.frameRate))
		throw new Error('Export frame rate must be 12, 15, 24, or 30 FPS.');
	if (settings.lottieMode !== 'vector' && settings.lottieMode !== 'raster')
		throw new Error('Unsupported Lottie export mode.');
	const frames = Math.round(settings.durationSeconds * settings.frameRate);
	if (settings.lottieMode === 'raster' && frames > maxRasterFrames)
		throw new Error(`Raster Lottie exports are limited to ${maxRasterFrames} frames.`);
...
export function validateNativeLottie(value: unknown, settings: ExportSettings): LottieDocument {
	...
	if (typeof value.v !== 'string' || !value.v)
		throw new Error('Lottie output requires a version string.');
	if (value.fr !== settings.frameRate || value.ip !== 0 || value.op !== frames)
		throw new Error("Lottie timing must match the project's duration and frame rate.");
	validateDimensions(value.w, value.h);
	if (!Array.isArray(value.layers) || value.layers.length > 500)
		throw new Error('Lottie output requires at most 500 layers.');
	if (value.assets !== undefined && (!Array.isArray(value.assets) || value.assets.length > 500))
		throw new Error('Lottie output supports at most 500 assets.');
	assertSize(value);
	return value;
}

export function buildRasterLottie(
	name: string,
	width: number,
	height: number,
	...
	if (frames.length !== validated.frames)
		throw new Error(`Expected ${validated.frames} raster frames, received ${frames.length}.`);
	if (frames.some((frame) => !frame.startsWith('data:image/png;base64,')))
		throw new Error('Raster frames must be embedded PNG data URLs.');

	const assets = frames.map((frame, index) => ({
		id: `frame_${index}`,
		w: width,
		h: height,
		e: 1,
...
function assertSize(value: unknown) {
	...
	try {
		...
		throw new Error('Lottie output must be JSON serializable.');
	}
	if (new Blob([json]).size > maxLottieBytes)
		throw new Error('Lottie output exceeds the 50 MiB limit.');
}
