...
export async function registerP5Tools(callbacks: Callbacks, signal?: AbortSignal) {
	...
	const tools = [
		tool(
			'get_sketch',
			'Get p5 sketch',
			'Return the applied source, editable configuration, schema, revision, runtime status, and Lottie export capability.',
			...
			() => {
				...
				return {
					...
					runtimeStatus: callbacks.getRuntimeStatus(),
					lottie: {
		...
		tool(
			'replace_sketch',
			'Replace p5 sketch',
			'Replace the complete p5.js source. Existing compatible config values are preserved.',
			{
				type: 'object',
			...
			async (input) => {
				...
				if (typeof value.source !== 'string' || !Number.isInteger(value.expectedRevision))
					throw new Error('A source string and expectedRevision are required.');
				const project = await callbacks.commands.replaceSource(
					value.source,
		...
		tool(
			'patch_sketch_config',
			'Patch sketch config',
			'Update schema-approved config paths without replacing sketch logic.',
			{
				type: 'object',
				additionalProperties: false,
				required: ['patches', 'expectedRevision'],
				properties: {
					expectedRevision: revision,
					patches: {
						type: 'array',
			...
			(input) => {
				const value = object(input);
				if (!Number.isInteger(value.expectedRevision) || !Array.isArray(value.patches))
					throw new Error('Patches and expectedRevision are required.');
				...
				return {
					ok: true,
					revision: project.revision,
					message: `${patches.length} config value(s) updated.`
				};
			}
		),
		tool(
			'control_sketch',
			'Control sketch playback',
			'Play, pause, or restart the current sketch.',
			{
				type: 'object',
				additionalProperties: false,
				required: ['action'],
				properties: { action: { type: 'string', enum: ['play', 'pause', 'restart'] } }
			},
			(input) => {
				const action = object(input).action;
				if (!['play', 'pause', 'restart'].includes(action as string))
	...
	return {
		...
		execute: async (input: unknown) => {
			try {
				...
				return {
					ok: false,
					category: message.startsWith('Revision conflict') ? 'conflict' : 'validation',
					message
				};
			}
		}
...
function patch(value: unknown): ConfigPatch {
	...
	if (!Array.isArray(input.path) || input.path.some((part) => typeof part !== 'string'))
		throw new Error('Every patch needs a string path array.');
