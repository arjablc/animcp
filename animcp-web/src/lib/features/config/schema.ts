...
export function parseDefinition(config: unknown, schema: unknown): SketchDefinition {
	...
	if (parsedSchema.type !== 'object')
		throw new Error('The root config schema must have type object.');
...
export function setConfigValue(
	...
	if (path.length === 0 || path.length > 8)
		throw new Error('Config paths must contain 1-8 segments.');
...
function parseSchema(value: unknown, depth = 0): SchemaNode {
	...
	for (const key of ['minimum', 'maximum', 'multipleOf'] as const) {
		if (
			...
			throw new Error(`Schema ${key} must be finite.`);
		if (typeof value[key] === 'number') node[key] = value[key];
	}
	if (node.multipleOf !== undefined && node.multipleOf <= 0)
		throw new Error('Schema multipleOf must be positive.');
	...
	if (node.type === 'object') {
		...
		node.properties = Object.fromEntries(
			entries.map(([key, child]) => {
				if (!key || key.length > 80 || dangerousKeys.has(key))
					throw new Error(`Unsupported config property: ${key}.`);
...
function validateValue(schema: SchemaNode, value: unknown, path: string): void {
	if (schema.enum && !schema.enum.includes(value as never))
		throw new Error(`${path} must be one of its allowed values.`);
	...
	if (schema.type === 'string') {
		if (typeof value !== 'string' || value.length > 2_000)
			throw new Error(`${path} must be a string under 2,000 characters.`);
		if (schema.format === 'color' && !/^#[0-9a-f]{6}$/i.test(value))
...
function schemaAtPath(schema: SchemaNode, path: string[]): SchemaNode {
	...
	for (const segment of path) {
		if (dangerousKeys.has(segment) || current.type !== 'object' || !current.properties?.[segment])
			throw new Error(`Unknown config path: ${path.join('.')}.`);
		current = current.properties[segment];
	}
	if (current.type === 'object' || current.readOnly)
		throw new Error(`Config path ${path.join('.')} is not editable.`);
	return current;
}
	...
	if (schema.type === 'object') {
		...
		return Object.fromEntries(
			Object.entries(schema.properties ?? {}).map(([key, child]) => [
				key,
				mergeNode(child, defaults[key], old[key])
			])
		) as ConfigObject;
	}
	try {
...
function collectControls(
	...
	path: string[],
	titles: string[],
	groups: Map<string, ControlField[]>
) {
	for (const [key, child] of Object.entries(schema.properties ?? {})) {
		...
		if (child.type === 'object') {
			collectControls(
				child,
				value[key] as ConfigObject,
				childPath,
				[...titles, child.title ?? humanize(key)],
				groups
			);
			continue;
		}
