...
const databaseName = "animcp-image-transport-test";
...
export type IntegrityStatus = "fixture-verified" | "self-checked" | "unverified";

export interface KnownFixtureSource {
...
export type ImportResult =
  ...
  | {
      ok: false;
      error: { category: ImportErrorCategory; message: string };
      durationMs: number;
    };
...
function waitFor<T>(operation: Promise<T>, signal?: AbortSignal): Promise<T> {
  ...
  return new Promise((resolve, reject) => {
    const onAbort = () => reject(
      signal.reason instanceof ImportProblem
        ? signal.reason
        : new ImportProblem("IMPORT_ABORTED", "The tool execution was cancelled.")
    );
...
function timedSignal(source?: AbortSignal) {
  ...
  const onAbort = () => controller.abort(
    source?.reason instanceof ImportProblem
      ? source.reason
      : new ImportProblem("IMPORT_ABORTED", "The tool execution was cancelled.")
  );
  if (source?.aborted) {
...
export function decodeBase64(data: string): Uint8Array {
  ...
  const decodedLength = normalized.length / 4 * 3 - padding;
  if (decodedLength > MAX_DECODED_BYTES) {
    throw new ImportProblem(
      "TOOL_ARGUMENT_TOO_LARGE",
      `Decoded image data exceeds the ${MAX_DECODED_BYTES / 1024 / 1024} MiB limit.`
    );
  }
...
export function inspectPng(bytes: Uint8Array) {
  ...
  if (
    ...
    throw new ImportProblem("INVALID_IMAGE", "The PNG dimensions exceed the experiment limits.");
  }
...
async function storeAsset(asset: StoredAsset, signal?: AbortSignal) {
  ...
  try {
    return await new Promise<{ asset: StoredAsset; alreadyImported: boolean }>((resolve, reject) => {
      ...
      const onAbort = () => {
        problem = signal?.reason instanceof ImportProblem
          ? signal.reason
          : new ImportProblem("IMPORT_ABORTED", "The tool execution was cancelled.");
        try {
          transaction.abort();
        } catch {
          // The transaction completed before cancellation could affect it.
        }
      };
      ...
      existingRequest.onerror = () => {
        problem = existingRequest.error ?? new Error("Could not read the existing asset.");
        transaction.abort();
      };
      existingRequest.onsuccess = () => {
        const existing = existingRequest.result as StoredAsset | undefined;
        if (existing) {
          if (existing.sha256 === asset.sha256) {
            outcome = { asset: existing, alreadyImported: true };
            return;
          }
          problem = new ImportProblem(
            "DUPLICATE_RETRY",
            "This transfer ID already belongs to a different image."
          );
          transaction.abort();
          return;
        }

        outcome = { asset, alreadyImported: false };
        store.add(asset);
      };
      ...
      transaction.onerror = () => {
        cleanup();
        reject(problem ?? transaction.error ?? new Error("Could not save the asset."));
      };
      transaction.onabort = () => {
        cleanup();
        reject(problem ?? new Error("Could not save the asset."));
      };
    });
  } finally {
    database.close();
...
export async function listAssets(): Promise<StoredAsset[]> {
  ...
  try {
    return await new Promise((resolve, reject) => {
      const request = database.transaction(assetStoreName).objectStore(assetStoreName).getAll();
...
export async function clearAssets() {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      const request = database.transaction(assetStoreName, "readwrite").objectStore(assetStoreName).clear();
...
export async function importAsset(
  ...
  options: {
    ...
    knownFixtureSource?: KnownFixtureSource;
  } = {}
  ...
  try {
    if (
      ...
      input.data.length > MAX_ENCODED_BYTES
    ) {
      return failure(
        startedAt,
        "TOOL_ARGUMENT_TOO_LARGE",
        `Encoded image data exceeds the ${Math.ceil(MAX_ENCODED_BYTES / 1024 / 1024)} MiB limit.`
      );
    }
    ...
    if (!parsed.success) {
      return failure(startedAt, "INVALID_INPUT", "The tool input does not match the import contract.");
    }
    ...
    if (options.knownFixtureSource && bytes.byteLength !== options.knownFixtureSource.expectedBytes) {
      return failure(startedAt, "PAYLOAD_TRUNCATED", "Decoded byte count does not match the copied fixture.");
    }
    ...
    if (options.knownFixtureSource && options.knownFixtureSource.sha256.toLowerCase() !== sha256) {
      return failure(startedAt, "HASH_MISMATCH", "Decoded image data does not match the copied fixture.");
    }

    const blob = new Blob([toArrayBuffer(bytes)], { type: parsed.data.mimeType });
    let decoded: { width: number; height: number };
    try {
      decoded = await waitFor(decodeImage(blob), signal);
    } catch {
      throwIfAborted(signal);
      return failure(startedAt, "INVALID_IMAGE", "The PNG could not be decoded by this browser.");
    }
    ...
    const saved = await storeAsset(
      {
        id: parsed.data.transferId,
        name: parsed.data.name ?? "Imported PNG",
    ...
    return {
      ok: true,
      asset: withoutBlob(saved.asset),
      durationMs: durationSince(startedAt),
      alreadyImported: saved.alreadyImported,
      integrity: options.knownFixtureSource
        ? "fixture-verified"
        : parsed.data.expectedBytes !== undefined && parsed.data.sha256 !== undefined
          ? "self-checked"
          : "unverified"
    };
  } catch (error) {
    if (error instanceof ImportProblem) {
      return failure(startedAt, error.category, error.message);
    }
