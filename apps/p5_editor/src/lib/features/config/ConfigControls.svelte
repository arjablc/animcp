      ...
      <label class:color-field={field.schema.format === "color"}>
        ...
        {:else if field.schema.format === "color"}
          <span class="color-control">
            <input type="color" value={field.value as string} oninput={(event) => onchange(field.path, inputValue(event))} />
...
<style>
  fieldset { border: 0; border-top: 1px solid var(--line); margin: 0; padding: 20px; }
  legend { color: var(--acid); font: 700 0.72rem/1 var(--mono); letter-spacing: 0.12em; padding-right: 10px; text-transform: uppercase; }
  ...
  .color-control code { color: var(--muted); font-family: var(--mono); }
