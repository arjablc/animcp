...
export function getProjects(): P5Project[] {
  try {
    ...
    return Array.isArray(value) ? value.map(migrateProject).filter((project): project is P5Project => !!project) : [];
  } catch {
    return [];
  }
}
...
export function migrateProject(value: unknown): P5Project | undefined {
  ...
  if (![1, 2].includes(project.version as number) || typeof project.id !== "string" || typeof project.name !== "string" || typeof project.source !== "string" || typeof project.revision !== "number" || !project.config || !project.schema) return undefined;
...
function normalizeExportSettings(value: unknown): ExportSettings {
  ...
  return {
    durationSeconds: typeof settings.durationSeconds === "number" && settings.durationSeconds >= 1 && settings.durationSeconds <= 10 ? settings.durationSeconds : defaultExportSettings.durationSeconds,
    frameRate: exportFrameRates.includes(settings.frameRate as ExportSettings["frameRate"]) ? settings.frameRate as ExportSettings["frameRate"] : defaultExportSettings.frameRate,
    lottieMode: settings.lottieMode === "raster" || settings.lottieMode === "vector" ? settings.lottieMode : defaultExportSettings.lottieMode,
  };
}
