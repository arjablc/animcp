<script lang="ts">
  ...
  onMount(() => {
    ...
    void tick().then(async () => {
      try { await commands.load(); nativeLottieSupported = commands.supportsNativeLottie(); }
  ...
  async function applySource() {
    applying = true;
    error = "";
    try { await commands.replaceSource(draft); nativeLottieSupported = commands.supportsNativeLottie(); }
    catch (cause) { report(cause); }
...
{#if project}
  <main class="editor-page">
    <header class="editor-bar">
      ...
      <div class="editor-actions">
        <Badge variant={runtimeStatus === "error" ? "destructive" : "secondary"} class={`runtime-pill${runtimeStatus === "error" ? " bad" : ""}`}>{runtimeStatus}</Badge>
        <Button variant="outline" size="sm" onclick={() => commands.control(runtimeStatus === "paused" ? "play" : "pause")}>{runtimeStatus === "paused" ? "Play" : "Pause"}</Button>
        <Button variant="outline" size="sm" onclick={() => commands.control("restart")}>Restart</Button>
        <Button variant="outline" size="sm" class="export png-export" onclick={exportPng}>Export PNG</Button>
        <Button variant="outline" size="sm" class="export" onclick={() => exportDialogOpen = true}>Export Lottie</Button>
      </div>
    </header>
      ...
      <div class:mobile-active={mobileTab === "preview"} class="panel preview-panel">
        ...
        {#if error}
          <Alert variant="destructive" class="error">
            ...
            <Button variant="ghost" size="icon-xs" aria-label="Dismiss error" onclick={() => error = ""}>×</Button>
          </Alert>
        {/if}
      </div>
  ...
  <LottieExportDialog bind:open={exportDialogOpen} {project} {commands} {nativeLottieSupported} ondownload={(blob) => download(blob, `${filename()}.json`)} />
{/if}
