...
describe("Lottie export", () => {
  ...
  it("builds one embedded image layer per raster frame", () => {
    ...
    expect((document.layers as Array<Record<string, unknown>>)[4]).toMatchObject({ refId: "frame_4", ip: 4, op: 5 });
  });

  it("caps raster frame counts", () => {
    expect(() => validateExportSettings({ durationSeconds: 10, frameRate: 30, lottieMode: "raster" })).toThrow("150 frames");
  });
});
