...
describe("p5 WebMCP", () => {
  it("registers the minimal tool surface", async () => {
    ...
    const commands = {
      load: vi.fn(), replaceSource: vi.fn(async () => ({ revision: 2 })), patchConfig: vi.fn(() => ({ revision: 2 })), control: vi.fn(), capture: vi.fn(), supportsNativeLottie: vi.fn(() => true),
    } as never;
    const result = await registerP5Tools({
      getProject: () => ({ source: "source", config: {}, schema: {}, revision: 1, exportSettings: { durationSeconds: 5, frameRate: 30, lottieMode: "vector" } } as never),
    ...
    expect(await tools[0].execute({})).toMatchObject({ ok: true, revision: 1, runtimeStatus: "running", lottie: { nativeSupported: true } });
  });
});
