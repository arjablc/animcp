...
export function TimelinePanel({ layers, selected, time, playing, onSelect, onTime, onPlaying, onChange }: { layers: Layer[]; selected: string; time: number; playing: boolean; onSelect: (id: string) => void; onTime: (time: number) => void; onPlaying: (playing: boolean) => void; onChange: (id: string, patch: Partial<Layer>) => void }) {
  ...
  return <section className="flex size-full min-h-0 flex-col bg-zinc-900">
    ...
    <div className="min-h-0 flex-1 overflow-auto">
      <div className="grid min-h-full min-w-[900px] grid-cols-[115px_1fr] sm:grid-cols-[190px_1fr] lg:grid-cols-[238px_1fr]">
        <div className="sticky left-0 z-20 border-r border-white/10 bg-zinc-900 pt-6">{tracks.map((layer) => <button key={layer.id} onClick={() => onSelect(layer.id)} className={cn("flex h-[42px] w-full min-w-0 items-center gap-2 px-3.5 text-left text-[9px] text-zinc-500", selected === layer.id && "bg-white/5 text-zinc-200")}><LayerIcon type={layer.type} /><span className="select-text truncate">{layer.name}</span></button>)}{tracks.length === 0 && <p className="px-3 py-6 text-[9px] text-zinc-600">Drag layers here</p>}</div>
        <div
          ref={timeline}
          ...
          onDrop={(event) => { event.preventDefault(); const id = event.dataTransfer.getData("application/x-animcp-layer"); const layer = layers.find((item) => item.id === id); if (!layer) return; const { rect, x } = timelinePosition(event.clientX); onChange(id, { inTimeline: true, start: trackStartFromPointer(x, rect.width, 0, layer.duration) }); onSelect(id); }}
        ...
        >
          ...
          {tracks.map((layer, index) => <div key={layer.id} className="relative h-[42px] border-b border-white/5"><button
            data-clip
