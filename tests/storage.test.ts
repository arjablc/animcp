...
describe('project migration', () => {
	it('adds export settings to version 1 projects', () => {
		expect(migrateProject(project)).toMatchObject({
			version: 2,
	...
	it('normalizes invalid saved export settings', () => {
		expect(
			migrateProject({
				...project,
				version: 2,
				exportSettings: { durationSeconds: 100, frameRate: 60, lottieMode: 'video' }
			})?.exportSettings
		).toEqual(defaultExportSettings);
	});
	...
	it('adds the fixed vector exporter only to untouched starter source', () => {
		...
		expect(
			migrateProject({ ...project, source: `${legacyDefaultSource}\n// user edit` })?.source
		).toContain('// user edit');
	});
});
